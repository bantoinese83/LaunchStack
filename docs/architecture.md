...
```
pnpm Workspaces
 ├── apps/web (Next.js 14 App Router) -> Marketing + Customer Web App
 ├── apps/mobile (Expo Router) -> iOS & Android Native App
 ├── apps/admin (Next.js 14 App Router) -> Internal Platform Admin Portal
 └── packages/
      ├── ui (Tailwind / Radix Web Primitives)
      ├── mobile-ui (React Native Primitives)
      ├── config (ESLint, TS, Tailwind shared configs)
      ├── types (Domain & Database TS Interfaces)
